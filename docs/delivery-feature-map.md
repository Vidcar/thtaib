...
Write these before treating the feature as specified: remaining OQ-006 items (durable shared index or store, restore capture gaps); live `memory=` / `skills=` UAT (built 2026-09-20, not `verified`); the memory write-through implementation PR (specified 2026-09-20, after loading, not built); the MCP implementation PR (ENV-007 specified 2026-09-20, not built); recorded-tool replay and remaining fail-closed codes before any STATE-006 `verified` claim; Electron UAT of the product Chat/Models/Knowledge surfaces; Builder canvas, node library and run inspector beyond the chrome ADR; access and connectors UX (OQ-011); task-case compare and export (OQ-014); remaining origin/IPC checks (OQ-002); run observability outside Lab (OQ-012); registry schemas (OQ-008); persistence convergence ADR (OQ-017). Host-shell approval flow for the first worker is specified in [environments and tools](../specs/modules/environments-tools.md). MCP expansion (browser and GitHub first) is specified there as ENV-007. [Issue #37](https://github.com/Vidcar/thtaib/issues/37) remains the tracking issue for end-to-end gaps.
