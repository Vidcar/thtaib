...
5. **Retrieval v1** — [STATE-006](../specs/modules/state-recovery.md#state-006) is `built` (Deep Agents retrieve-and-offload over a derived `InMemoryVectorStore`; fail closed without a loaded `embedding: on` deployment). Not `verified`. Do not write a second knowledge store. Remaining [OQ-006](../specs/open-questions.md#oq-006): durable shared index, automatic writes, restore capture gaps. David-PC already has the dedicated embedding GGUF; the operator must register and start it as a deployment.
