...
- **Custom adapter** — pass an `AgentServerAdapter` instance. The hook delegates every command and subscription to the adapter. See [Transports](https://github.com/langchain-ai/langgraphjs/blob/ec67d5d70dc26341e92a0962d9d2f4018c310b39/libs/sdk-react/docs/transports.md).
...
| `optimistic`    | `boolean`                                                                                 | Echo `submit()` input into `values` / `messages` immediately and reconcile by id as the server streams back. Defaults to `true`. Set `false` for server-authoritative-only. See [v1 migration §5.4](https://github.com/langchain-ai/langgraphjs/blob/ec67d5d70dc26341e92a0962d9d2f4018c310b39/libs/sdk-react/docs/v1-migration.md). |
...
`stream.hydrationPromise` settles when the thread's initial hydration resolves (or rejects). The same promise is exposed on every render — a fresh one is installed every time the hook binds to a new `threadId`. [`useSuspenseStream`](https://github.com/langchain-ai/langgraphjs/blob/ec67d5d70dc26341e92a0962d9d2f4018c310b39/libs/sdk-react/docs/suspense.md) leans on this directly. In plain `useStream` apps you rarely need it, but it's useful for server-rendered-then-hydrated views that want to show a skeleton until the first snapshot lands:
...
- [Transports](https://github.com/langchain-ai/langgraphjs/blob/ec67d5d70dc26341e92a0962d9d2f4018c310b39/libs/sdk-react/docs/transports.md)
- [Interrupts & headless tools](./interrupts.md)
- [Type safety](https://github.com/langchain-ai/langgraphjs/blob/ec67d5d70dc26341e92a0962d9d2f4018c310b39/libs/sdk-react/docs/type-safety.md)
