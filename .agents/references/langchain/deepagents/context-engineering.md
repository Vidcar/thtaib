...
See [Overview](https://docs.langchain.com/oss/python/deepagents/overview#execution-environment) for built-in capabilities and [Customization](./customization.md#tools) for passing tools directly.
...
To shrink the tool schemas sent on every turn before compression ever runs, exclude unused built-in tools via a [harness profile](https://docs.langchain.com/oss/python/deepagents/profiles#harness-profiles) (`excluded_tools`). See [Tool prompts](#tool-prompts).
