...
[Working rules](../AGENTS.md) · [Contract index](README.md) · [Status and evidence](catalog.json) · [Decisions](decisions/README.md)

This document defines boundaries for [the thtaib vision](../thtaib-vision.md): Models, Chat, Lab and Workflows share model/configuration records, evidence, executions and artifacts. **Agent run / Builder** are existing UI and legacy contract terminology for Workflows. Implementation status lives in [the catalogue](catalog.json), with mismatches in [deviations](deviations.md); [Revision 0.5](sources/README.md) is historical provenance.
...
- **Workers**: the first worker is the Windows host shell with approvals. Isolated browser, graphical, WSL and Docker workers are later integration boundaries. Any future container-service orchestration must not become an application job scheduler. Chat browser via MCP is not a worker ([ENV-007](modules/environments-tools.md#env-007)).
- **MCP servers**: optional extra tool sources under [ENV-007](modules/environments-tools.md#env-007); stdio servers are backend child processes and HTTP servers are connected endpoints the workbench does not host. MCP is not a worker environment or isolation. Core Chat runs with MCP absent ([ARCH-007](#arch-007)).
...
- A working directory is not a security boundary; MCP is not isolation. Host-shell and MCP approvals reuse the framework interrupt under [ENV-002](modules/environments-tools.md#env-002) and [ENV-007](modules/environments-tools.md#env-007). Remaining worker questions are [OQ-003](open-questions.md#oq-003); a durable Approvals inbox is [OQ-011](open-questions.md#oq-011).
...
- **Knowledge refs are loaded content; retrieval is a derived index.** Selected versions load through the framework mechanisms in [AGT-004](modules/agents-workflows.md#agt-004); durable ownership and write policy live in [STATE-005](modules/state-recovery.md#state-005), optional retrieval in [STATE-006](modules/state-recovery.md#state-006). Neither the vector index nor materialized harness files become a second knowledge owner. Retrieval requires a loaded dedicated embedding deployment, not merely a GGUF file on disk; missing embedders fail closed.
