...
Record families: **Model bundle**, **Run profile**, **Running deployment**, **Compatibility record**, and the model-related part of a **Run record**. Routes under `/v1`: `bundles` (list, get, `inspect`, `compatibility`), `imports/huggingface`, `imports/local`, `imports/{job}`, `profiles` (CRUD), `settings/preview`, `runtime`, `runtime/pin`, `deployments` (`managed`, `connected`, `start`, `stop`, `detach`, `health`, `smoke`) and `compatibility` (`records`, `assess`, `records/{id}/overrides`). Module-local Pydantic models define the shapes; they are not yet on the shared-contract generation path ([contracts](../contracts.md)).

## Behaviour

- **Runtime pin.** The supported managed runtime is llama.cpp **b11045**, Windows CUDA 13.4 assets extracted into `runtimes\`. NVIDIA absent is a clear error, not a silent CPU fallback. A `llama-server` on PATH is unsupported. Pin while running is refused or stops managed servers first. Non-Windows CUDA is not a supported path.
- **Settings bags.** Startup keys map to `llama-server` flags (`ctx_size`, `n_gpu_layers`, `threads`, `parallel`, `batch_size`, `ubatch_size`, `flash_attn`, `alias`, host/port). `flash_attn` is a valued enum serialised `--flash-attn on|off|auto`; a bare `--flash-attn` is never emitted. Per-request keys are the sampling and limit controls the adapter forwards. Unknown keys are unsupported; accepted but untested values are unverified. The current `mlock`/`no_mmap` mapping is a known defect ([DEV-002](../deviations.md#dev-002)).
- **Default GPU profile.** `ctx_size` 65536 (3090 / 24 GB), `n_gpu_layers: -1`, `flash_attn` valued. Desktop Start binds this profile; empty startup is not the product default.
- **Managed deployment lifecycle.** Start, stop, health and reconcile are serialised per deployment. A live record stores `process_identity` (`pid`, `create_time`, `executable`) and destructive stop verifies it first; a stale or reused PID is refused and the record cleared without killing the unmatched process. A duplicate start of a verified-owned deployment returns the existing record. A process that exits while another process answers the endpoint is `failed`, not owned. Companion `mmproj` files must be passed to the server when the bundle has one (currently not done, [DEV-002](../deviations.md#dev-002)).
- **Connected endpoint.** Attaches an existing OpenAI-compatible server with `scope=connected`; health is probed; start, stop and kill are refused; detach only.
- **Compatibility records.** Versioned records with requirements, supported capabilities and controls, recommendations, sources and validation evidence. `publisher_guidance`, `tested_adjustments` and `user_overrides` are separate lists. Support status is `unverified` | `known_incompatible` | `tested`. An unfamiliar model is `unverified`, `usable=true`; it is never treated as incompatible and never refused a deployment for being unverified. Prefer reading `llama-server` `/props` for reported capabilities over hand-maintained fixtures where the pinned version provides them.
...
Rows MOD-001…006 in [the catalogue](../catalog.json). Managed start on real hardware has no live evidence yet; it is the first build-order proof.
