...
- **Multimodal companion:** a bundle whose classified companions include an `mmproj` GGUF starts with `--mmproj <path>` after `-m <primary>`. A recorded projector missing on disk is a clear `bundle_file_missing` failure, not a start. This is always-on by owner decision: importing the companion into the bundle is the expression of vision intent, so no profile key is needed to opt in or out.
