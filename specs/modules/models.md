...
- **Retired keys (corrected against the pinned b11045 binary):** the earlier `mlock → --mlock` and `no_mmap → --no-mmap` mapping was wrong — b11045 answers `error: invalid argument` to both. A profile or request that still carries `mlock` or `no_mmap` gets those keys listed in the startup bag's `unsupported` list plus a `retired` note naming the `load_mode` value to use instead. They are never emitted and never silently dropped. Saved profiles are re-resolved when a deployment is created, so an old profile fails visibly rather than crashing llama-server.
- **Multimodal companion:** a bundle whose classified companions include an `mmproj` GGUF starts with `--mmproj <path>` after `-m <primary>`. A recorded projector missing on disk is a clear `bundle_file_missing` failure, not a start.
