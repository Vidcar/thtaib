...
- **Settings bags.** Startup keys map to `llama-server` flags (`ctx_size`, `n_gpu_layers`, `threads`, `parallel`, `batch_size`, `ubatch_size`, `flash_attn`, `load_mode`, `alias`, host/port). All are valued; there are no bare flags. `flash_attn` serialises `--flash-attn on|off|auto`; `load_mode` serialises `--load-mode auto|none|mmap|mlock|mmap+mlock|dio`. The retired keys `mlock` and `no_mmap` (invalid on b11045) are never emitted and never silently dropped: a profile or request carrying them lists them in the startup bag's `unsupported` list with a `retired` note naming the `load_mode` value to use, and saved profiles are re-resolved from their requested keys on deployment create and Chat resolve so old profiles still start. Per-request keys are the sampling and limit controls the adapter forwards. Unknown keys are unsupported; accepted but untested values are unverified.
