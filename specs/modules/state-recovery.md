...
- **History ≠ project ≠ thread.** Transcripts are presentation. Filesystem tools write project storage only; harness-internal files (`/large_tool_results/`, `/conversation_history/`, `/retrieved/`, `/memories/`, `/skills/`) stay under the product data root (see [agents and workflows](agents-workflows.md#behaviour)). Editing or clearing a transcript restores or deletes no file and does not fork or reset the thread. Missing conversation → thread → run linkage after restart is an explicit gap, never a silent new thread.
...
- **Durable knowledge.** Append-only versions with provenance; revert creates a new version. Writes name an expected `base_version`; a mismatch is `knowledge_conflict`. Protected instructions reject agent-origin writes; a human or API-maintainer path may edit them with provenance. Automatic agent writes need an explicit scope policy and carry actor and run id. The named policy is: a successful live-tool official `edit_file` / `write_file` on `/memories/**` write-throughs kind `memory` into this store (`actor=agent`, `run_id` set) so Knowledge shows the new version. HTTP agent-origin `/v1/knowledge` writes still require `scope_policies`. Skills stay non-writable. Context-capture retention duration and redaction mode (`redact_secrets` by default, retain plaintext, or discard) are local configuration and apply to persisted diagnostic copies of model requests before storage; transcripts, checkpoints and operational events follow a separate retention policy. Selected versions are loaded as content for a run (STATE-005): memory through Deep Agents `memory=`, skills through `skills=`, protected instructions through `system_prompt=`. Derived `/memories/` and `/skills/` files live on the harness composite and are discarded with the run; the durable copy of a memory edit is the new version, not the scratch file. Query-time retrieval is a derived LangChain index over that content, not a second store (STATE-006). Write-through is specified, not implemented.
- **Memory write safety.** Recorded-tool replay never writes through to durable Knowledge. Identical content appends no version; a `knowledge_conflict` never overwrites the competing version. HTTP agent-origin writes continue to require `scope_policies[scope].automatic_agent_writes` (default false); the harness policy does not flip that default or enable skill write-through.
- **Persistence split.** [state/migrate.py](../../apps/backend/src/workbench_backend/state/migrate.py) performs the existing run/Chat JSON-to-SQLite startup cutover and archives the migrated JSON, without dual writes. Bundles, profiles, deployments, import jobs, compatibility, Lab and knowledge metadata still use component JSON stores; their remaining migration is [OQ-017](../open-questions.md#oq-017). Knowledge bodies remain files.
...
[OQ-004](../open-questions.md#oq-004) identities, event order, exactly-once; [OQ-005](../open-questions.md#oq-005) snapshot retention and environment snapshots; [OQ-006](../open-questions.md#oq-006) remainder (durable shared index or store, restore capture gaps) after the v1 retrieval, memory/skills-loading, and write-through recommendations; [OQ-017](../open-questions.md#oq-017) persistence strategy.
