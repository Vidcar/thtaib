...
- **History ≠ project ≠ thread.** Transcripts are presentation. Filesystem tools write project storage only; harness-internal files must not land there either ([DEV-004](../deviations.md#dev-004)). Editing or clearing a transcript restores or deletes no file and does not fork or reset the thread. Missing conversation → thread → run linkage after restart is an explicit gap, never a silent new thread.
