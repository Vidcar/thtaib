...
- **MCP scope.** ENV-007 excludes prompts/resources/sampling/roots, MCP Apps, Docker-hosted GitHub, an OAuth app, unpinned `@latest`, GitHub `all`/insiders modes and attachment to the user's everyday Chrome profile. These exclusions apply to that initial integration, not permanent product exclusions. Pin-specific adapter rationale is in [upstream references](../sources/upstream.md#langchain-mcp).

- **Today.** The enabled catalogue is visibility tools (`echo`, `time_now`), Deep Agents filesystem tools bound to project storage, and the host-shell `execute` tool when a project (cwd) is bound. No MCP client, MCP server record, browser, graphical, ComfyUI, MCP-Apps or interpreter path exists. Recorded-tool replay in the Lab attaches no live project, host-shell, or retrieval backend; knowledge routes may use scratch so official memory/skills middleware can `download_files`.
- **Intended.** Filesystem tools target project storage. Official `/memories/**` writes on a live-tool run are knowledge write-through (STATE-005), not project files; `/skills/**` writes stay denied. Shell and later isolated browser/graphical tools require a connected worker in a declared environment that explains where it runs, which files it exposes, what it may install and what isolates it. Chat browser access in this slice is the Playwright MCP product server ([ENV-007](#env-007)), not that isolated worker.
