...
- **Present surfaces.** Models, Deployments and debug-quality Chat, plus optional Agent-run, Lab and Knowledge debug panels that show raw records. Chat and Agent-run show a host-shell Approve/Deny card when `pending_interrupt` is set (Deep Agents `interrupt_on`, not a durable inbox). Chat is not finished polish; Builder is not shipped.
