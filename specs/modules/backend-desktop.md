...
- **Present surfaces.** Chat, Models (bundles, profiles and deployments) and Knowledge are product surfaces with empty and error states. Chat works without a project folder; file tools and the host shell stay unavailable until one is bound. Files and images are intended Chat inputs as the relevant model/runtime support and attachment path are delivered; no existing attachment path is implied by this wording. Knowledge lists current STATE-005 versions (including agent write-through once that slice is built) from `GET /v1/knowledge/entries` and should refresh when the surface is shown. Agent-run is a one-off harness surface. Lab stays reachable for capture/restore/rerun and does not pretend Model Lab runners or a science-app restyle exist. Chat and Agent-run show a host-shell Approve/Deny card when `pending_interrupt` is set (Deep Agents `interrupt_on`, not a durable inbox). Live run progress streams over SSE. Builder and voice are not shipped; voice remains an intended integration covering transcription, speech output and conversational interaction through established local components.
