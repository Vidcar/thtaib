...
- **Present surfaces.** Chat, Models (bundles, profiles and deployments) and Knowledge are product surfaces with empty and error states. Chat works without a project folder; file tools and the host shell stay unavailable until one is bound. Agent-run is a one-off harness surface. Lab stays reachable for capture/restore/rerun and does not pretend Model Lab runners or a science-app restyle exist. Chat and Agent-run show a host-shell Approve/Deny card when `pending_interrupt` is set (Deep Agents `interrupt_on`, not a durable inbox). Live run progress streams over SSE. Builder is not shipped.
