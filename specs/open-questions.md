...
Open: executable and network access and credential delivery; what the host shell exposes versus what later environments isolate; cancellation and teardown truth beyond reject-then-cancel on an interrupt; worker identity for later remote environments. MCP is discovery and invocation, not isolation. A durable Approvals inbox remains [OQ-011](open-questions.md#oq-011). **Evidence needed:** a real task through the host shell with an approval granted and one denied, plus an attempt to exceed the approved boundary, on David-PC; test explicitly authorised host access separately from isolated execution when later environments arrive.
