...
**Status:** partially decided (knowledge store #17; capture privacy #64; content loaded before run #57). **Research recorded 2026-09-19:** query-time RAG **is** in the first usable version, delivered through LangChain / Deep Agents retrieval components with little custom code ([changelog](decisions/changelog.md#2026-09-19--oq-006-retrieval-research-include-rag-in-v1); product owner's 2026-09-19 "if" is satisfied). Specified as [STATE-006](modules/state-recovery.md#state-006) (`planned`). **Owner:** persistence/agent boundary. **Blocks:** automatic durable-knowledge writes without a scope policy; a decision that a *durable* retrieval index is shared across surfaces; implementing a second knowledge store or a workbench-written retriever.

v1 shape: agentic retrieve-and-offload — `RecursiveCharacterTextSplitter`, `InMemoryVectorStore`, `OpenAIEmbeddings` against llama-server `POST /v1/embeddings`, a LangChain `@tool` that `similarity_search`es and writes chunks to `/retrieved/` on the existing `CompositeBackend` (harness scratch, never the project). The STATE-005 store stays the durable source; the vector index is derived per run. Retrieved documents are neither durable project memory nor training. Deep Agents `memory=` / `skills=` are always-load / progressive disclosure, not RAG; they remain the preferred replacement for today's system-prompt append (AGT-004), as a nearby slice, not as STATE-006.

Still open: whether a durable index is ever shared across Chat, Lab and Builder (v1 creates none); what capture gaps a restored run must report when retrieval ran; which dedicated embedding GGUF is the product default. **Evidence needed:** a retrieval path that is not a second knowledge owner, and a redacted, retained capture on a real David-PC workload with a loaded embedding deployment.
