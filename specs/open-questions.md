...
**Status:** **partial.** Specification-integrity commands remain registered. Backend unittest and desktop type-check/build are registered local commands and GitHub Actions jobs; exact status-check names are in [commands CI scope](commands.md#ci-scope). Those jobs exist and execute the registered product commands as **advisory CI only**; they are not catalogue `verified` evidence, not build-stage acceptance, and not merge-blocking required checks. This does not close the question.

**Hosting / required-check decision (locked, [Issue #36](https://github.com/Vidcar/thtaib/issues/36)):** the repository stays **private** on free personal GitHub. Classic branch protection and rulesets return **403** without Pro or making the repository public. David will **not** upgrade to Pro. Required status checks are deferred indefinitely until the repository is on Pro or public. There is **no open ask for David** to enable required checks, default-branch protection, or required CODEOWNERS review. Do not list those settings as pending human work, and do not frame a Pro upgrade as a product requirement.
