...
**Status:** partially decided (knowledge store #17; capture privacy #64; content loaded before run #57; retrieval v1 built 2026-09-20; memory/skills loading specified 2026-09-20). **Research recorded 2026-09-19:** query-time RAG **is** in the first usable version, delivered through LangChain / Deep Agents retrieval components with little custom code ([changelog](decisions/changelog.md#2026-09-19--oq-006-retrieval-research-include-rag-in-v1); product owner's 2026-09-19 "if" is satisfied). Implemented as [STATE-006](modules/state-recovery.md#state-006) (`built`, not `verified`). **Research recorded 2026-09-20:** Deep Agents `memory=` / `skills=` replace the memory/skill system-prompt append ([changelog](decisions/changelog.md#2026-09-20--oq-006-memory-and-skills-replace-prompt-append); [AGT-004](modules/agents-workflows.md#agt-004), [STATE-005](modules/state-recovery.md#state-005)). Loading is specified, not implemented, not `verified`. Catalogue rows stay `built`. **Owner:** persistence/agent boundary. **Blocks:** automatic durable-knowledge writes without a scope policy; a decision that a *durable* retrieval index or Deep Agents store is shared across surfaces; implementing a second knowledge store or a workbench-written retriever or memory middleware.
...
Still open: whether a durable retrieval index or Deep Agents store is ever shared across Chat, Lab and Builder (v1 creates none); what capture gaps a restored run must report when retrieval or `memory=` / `skills=` ran; whether agent `edit_file` on `/memories/` ever becomes a STATE-005 version. **Evidence recorded:** David-PC Chat HTTP fail-closed (missing / chat-as-embedder / unloaded) and live `search_knowledge` writing `/retrieved/` under harness scratch at `7db7f45` ([evidence](evidence/2026-09-20-david-pc-retrieval.md)). **Evidence still needed:** recorded-tool replay without a live index; remaining fail-closed codes; Electron selectors; Lab / Agent-run HTTP; live `memory=` / `skills=` loading (not yet implemented). Unit tests with `DeterministicFakeEmbedding` are not retrieval evidence. Catalogue STATE-006, STATE-005 and AGT-004 stay `built`.
