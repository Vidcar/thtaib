...
Runs the backend unittest modules, including model-manager API tests for bundles, GGUF inspect, settings bags and deployments, Issue #21 runtime/pin/`flash_attn` valued-enum checks, Issue #31 MOD-006 provenance-separation and unverified≠incompatible checks, harness/adapter tests for AGT-001/002/005/006 and MOD-005, Chat→harness wiring plus STATE-002 transcript≠project and filesystem-tools→project-storage checks, STATE-001 dual `application.sqlite` / `checkpoints.sqlite` path and JSON-linkage migration plus restart follow-run-to-checkpoint-id checks, STATE-004 unknown-effect safety (no silent replay; no external-effect rollback promise), Issue #42 cancel honesty (request→`cancel_requested`; confirm→`cancelled`; `cancel_requested` is not quiescent), Lab capture/restore/rerun plus engine-unavailable checks for LAB-001…004 and STATE-003, durable-knowledge checks for STATE-005 (create/edit/revert/conflict/protected-deny/retention-redaction and knowledge refs from Lab/harness), Issue #35 WF-001 definition-compiler checks (mixed configuration/workflow compile; configuration links are not executable steps), and Issue #40 local-trust checks (shared-secret file under `state\`, loopback bind only, missing token → 401 and wrong token → 403 on privileged `/v1` routes including Chat/Lab/project-file ops). These are executable unit checks, not David-PC UAT and not catalogue `verified` evidence.
