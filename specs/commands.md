...
Runs the backend unittest modules, including model-manager API tests for bundles, GGUF inspect, settings bags and deployments, Issue #21 runtime/pin/`flash_attn` valued-enum checks, harness/adapter tests for AGT-001/002/005/006 and MOD-005, Chat→harness wiring plus STATE-002 transcript≠project and filesystem-tools→project-storage checks, STATE-001 dual `application.sqlite` / `checkpoints.sqlite` path and JSON-linkage migration plus restart follow-run-to-checkpoint-id checks, Lab capture/restore/rerun plus engine-unavailable checks for LAB-001…004 and STATE-003, and durable-knowledge checks for STATE-005 (create/edit/revert/conflict/protected-deny/retention-redaction and knowledge refs from Lab/harness). These are executable unit checks, not David-PC UAT and not catalogue `verified` evidence.
