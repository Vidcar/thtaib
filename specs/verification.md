...
[The catalogue](catalog.json) links each requirement to its authoritative specification and separately records `implementation_status`, `code`, `tests` and `evidence`. All product requirements start `unassessed` because no repository was inspected.

After inspection, use `planned` for absent work or `partial` for incomplete/insufficiently evidenced work. Use `verified` only for the current scope with actual implementation/test paths and passing evidence. Preserve a retired requirement's ID/heading and explain its replacement rather than delete or reuse it.
