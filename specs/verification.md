...
Known smoke quirk, recorded so it is not mistaken for framework behaviour: the tiny model writes its file under `/large_tool_results/`, which the current bare `FilesystemBackend` places inside the project ([DEV-004](deviations.md#dev-004)); the smoke asserts only that the written file is inside the project. The `CompositeBackend` change must adjust the smoke's write task in the same change.
...
Each gate is added with the first implementation it protects and registered in [commands](commands.md): unit tests and the desktop type-check/build (registered, required on `main`); shared-contract freshness (registered, required); real-model smoke (registered; binding `integration-tests` → `apps/backend/tests_integration`; runs on every PR, not yet a required check); import-boundary check (unbound); managed Windows CUDA deployment, worker and MCP integration tiers (do not exist); permission, cancellation and recovery checks for workers; crash and no-duplicate-continuation checks for durable runs; consistent-capture and isolated-restore checks for snapshots. A specification-only workflow is never the sole gate once code exists.
