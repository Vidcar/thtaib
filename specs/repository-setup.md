...
[real-model-smoke.yml](../.github/workflows/real-model-smoke.yml) adds `real-model-smoke (ubuntu-latest)` ([commands](commands.md#real-model-smoke)). It runs on every pull request and is intended to join the required list above; adding the context to branch protection is a maintainer action. Record it in the list above once that is done.
