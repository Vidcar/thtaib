...
- Live project-bound runs attach `LocalShellBackend(root_dir=project, virtual_mode=True, inherit_env=True)` as the composite default. `execute` is in the enabled catalogue and presented only when a project (cwd) is bound. Requesting it without a project is `shell_requires_project` (400). No home-directory default.
