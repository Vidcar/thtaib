...
Agent tasks SHALL run through Deep Agents using LangChain components and LangGraph. Chat SHALL call this harness directly with or without a bound project folder. The application MUST NOT add its own model-and-tool loop.

#### Scenario: Project-bound and project-free chat

- WHEN Chat completes a real file-editing task with a project bound
- THEN the execution path MUST show the harness owns model/tool iteration
...
Host-shell and MCP approvals SHALL use Deep Agents `interrupt_on` and LangGraph resume on the same checkpointer thread. A pending interrupt SHALL keep the run `running` with recorded interrupt details; it MUST NOT create a separate lifecycle status. Cancel while interrupted SHALL reject-resume the pending command and finish as `cancelled` only after the worker confirms stop.

#### Scenario: Approve deny cancel

- WHEN a dangerous command pauses on an interrupt
- THEN Approve or Deny MUST resume through the framework decision mechanism
- AND Cancel MUST resolve the pending command without silently replaying or claiming stopped external work before confirmation.
