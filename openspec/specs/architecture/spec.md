...
The application SHALL use llama.cpp for supported inference, Deep Agents for each agent loop, LangGraph for runtime checkpoints and outer workflows, and LangChain for model, message, and tool interfaces. The application MUST own configuration, lifecycle, visibility, and connecting contracts, and MUST NOT implement a second model/tool loop or workflow runtime.
