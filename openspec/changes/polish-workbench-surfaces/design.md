...
5. A quiet, idempotent consolidation selects the healthy owned configuration as default (otherwise most recent saved), merges exact equivalent configurations ignoring automatically assigned ports, and preserves distinct variants and historical snapshots. Merged records retain their original identity and bags for history, with one canonical owner for live resolution. Existing profile instructions remain preserved and named in effective setup; ordinary model saves cannot erase them. New configurations accept loading and response settings only, with new instruction authoring in Agents/setup. This preserves authored behaviour without inventing synthetic agents or a second live resolver.
