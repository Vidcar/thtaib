...
Implementation status (2026-09-21): in progress on `codex/packet-03-shared-chat`. Latest backend default passes 472 tests; integration 149 passed. Contracts are fresh and OpenSpec passes 15/15. Full desktop build passed after Stop, delayed/stale/new-run ownership fixes and idle stopped-model wording correction. Built-Windows full/half-screen checks corrected sidebar/composer layout and model loading/status. Real Qwen continuation across reopen/restart retained context; streamed Stop retained partial output. Loading-time Stop reached Cancelled with zero model/tool requests and no replay. The reported popup was a repaired test-fixture error; failures now exit through logs. Review also fixed stale typed-answer authority, exact retained-output attribution, unused asset deletion and scoped attachment writes. Disposable everyday test conversation/runs/checkpoint were removed; model weights and other conversations remain intact. Advanced components still need shared-shell integration. Dave's early arrangement accept/change/defer question remains pending; neither UX gate is resolved. Final Windows/tool journeys, final checks, Git/merge and rebuilt delivery remain. Task boxes stay open until complete scope is verified.
