...
Use the repository's supported official LangChain MCP adapter and one shared async harness driver. Migrate invocation/streaming, all middleware hooks, saver/state access, resume, cancellation and shutdown as one compatible path. Reuse the existing checkpointer database and public APIs; keep long-lived loop-bound resources on their owning loop. Ordinary Chat and Lab must still work without MCP.
