...
- [x] 1.1 Implement immutable project/non-project session binding, new/rename/archive/search/reopen and stable per-turn threads/runs with frozen setup.
- [x] 1.2 Persist drafts and editable queued turns, advance only after success, and revalidate setup/permissions/admission at dispatch.
...
- [x] 1.8 Complete cancellation, tray/reopen and explicit Quit handling; preserve partial results, safe rendering and accessible existing controls.
...
- [x] 2.1 Test same-thread continuation and fresh-session isolation; reject session moves; verify branches retain area and historical removed-project identity.
- [x] 2.2 Test queue edits/navigation/restart, selector changes, approval/input waits, automatic success advance and failure/cancel pause.
- [x] 2.3 Test empty tools in sync/async/restore hooks, unexpected calls, four approval scopes, revoked grants, mixed ordered decisions and stale/duplicate/wrong-run answers.
- [x] 2.4 Test SDK-scoped reconnect/resynchronization, late events and terminal hydration with no duplicated or cross-conversation content; retain partial text and actual tool results against durable application history.
- [x] 2.5 Test branch heads, supported snapshot integrity, answer-only regeneration without tool effects, explicit retry disclosure and crash boundaries around effects/approval acceptance.
- [x] 2.6 Test denied/failed writes not becoming artifacts, changed mutable files, scoped attachments with tools off, safe HTML/code rendering and capacity failures without silent truncation.
...
- [x] 2.8 Run a real local-model conversation across reopen/restart, a disposable file task with approval/denial/cancel, and Windows drag/drop, scrolling, tray and Quit checks.
...
- [x] 3.1 Demonstrate the basic Chat arrangement early after 1.1-1.3 and the basic 1.9 shell, including Models -> Chat selection/install/settings/start/send/cancel/reopen; resolve requested layout changes before accumulating controls or record Dave's explicit review deferral.
- [x] 3.2 Demonstrate the completed Chat/history/queue/approval/files/recovery journey in the built Windows application using full and half-screen windows, Windows scaling, keyboard navigation, long content and a failure/recovery state; record actual conditions and technical results separately from Dave's acceptance or explicit deferral in `design.md`/PR. Resolved 2026-09-22: Dave accepted project-file attach/send and the full-screen review. Document-source inspection, memory-proposal review, the skill journey, the half-screen review, and leftover fixture cleanup are deferred, not accepted.
...
Implementation status (2026-09-22): Dave accepted actual Explorer drop, Windows notification activation and tray reopening; these close their respective technical gates. The adjoining Models/Library/Knowledge/Settings repairs are implemented with Packet 04: repository selection loads actual files; downloads expose progress/failure/retry; saved capability evidence distinguishes current/stale/untested; image companions are explicitly selected; installed model detail follows the selected model; exact file deletion previews name affected records/files. Native image picker → retained thumbnail → viewer → Gemma Chat passed, and Qwen's actual paired-image probe passed with its selected projector. The rebuilt desktop passes its full typecheck/component/Electron trust checks. Final combined backend verification passed 612 default tests and 156 integration tests with no failures or reported skips; the full desktop build and strict OpenSpec 16/16 also pass. Task 3.2 is resolved by Dave's partial acceptance and explicit deferral. Do not treat the deferred document-source, proposal, skill, half-screen, or fixture-cleanup work as accepted.
...
Final follow-up repairs cover canonical project attachment scope, retained drafts after rejected submission and navigation/New Chat surviving a failed initial history load. Actual-component regressions pass. Dave confirmed project-file attach/send works. The deferred journeys move to a later change.
