...
class InteractionStoreMixin:
    ...
    def append_interaction(self, thread_id: str, events: list[dict[str, Any]], *,
                           snapshot: dict[str, Any] | None = None, run_id: str | None = None) -> int:
        ...
        with self._lock:
            ...
            try:
                for original in events:
                    ...
                    self._conn.execute("INSERT INTO interaction_events(thread_id,seq,payload) VALUES(?,?,?)",
                                       (thread_id, seq, json.dumps(event)))
    ...
    def interaction_page(self, thread_id: str, since: int) -> tuple[list[dict[str, Any]], int]:
        """Read the replay page and its high-water mark in one lock scope."""
        with self._lock:
            page = self.interaction_events_after(thread_id, since)
            binding = self.get_interaction(thread_id)
            return page, binding["seq"] if binding else 0
