...
def read_skill_package(source: Path) -> tuple[str, dict[str, bytes]]:
    ...
    try:
        if source.is_dir():
            ...
            raise KnowledgeError("Choose a skill directory, Markdown file or ZIP archive.", code="skill_package_missing", status_code=400)
    ...
    try:
        markdown = files["SKILL.md"].decode("utf-8-sig")
        match = re.match(r"^---\s*\n(.*?)\n---\s*(?:\n|$)", markdown, re.DOTALL)
    ...
    if not isinstance(metadata, dict) or not isinstance(metadata.get("name"), str) or not re.fullmatch(r"[a-z0-9]+(?:-[a-z0-9]+)*", metadata["name"]) or len(metadata["name"]) > 64 or not isinstance(metadata.get("description"), str) or not metadata["description"].strip() or len(metadata["description"]) > 1024:
        raise KnowledgeError("SKILL.md requires a valid lowercase name and a description of at most 1024 characters.", code="skill_frontmatter_invalid", status_code=400)
