...
export function ResponseSettingsEditor({ value, onChange, facts, options, disabled = false, compact = false }: {
  value: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void;
  facts: Record<string, EffectiveSetting>; options: BundleConfigurationOptions | null; disabled?: boolean; compact?: boolean;
}) {
  ...
  return <div className="response-settings-editor">
    {modes?.supported ? <div className="setting-row"><CompactSwitch label="Thinking" checked={effectiveMode === "on" || effectiveMode === true} onChange={enabled => patch("reasoning", enabled ? "on" : "off")} disabled={disabled || effectiveMode == null} description={effectiveMode == null ? "Default not reported. Choose an explicit value below." : source("reasoning")} />
      {effectiveMode == null ? <select aria-label="Thinking" value={String(value.reasoning ?? "auto")} disabled={disabled} onChange={event => patch("reasoning", event.target.value)}><option value="auto">Model default · unknown</option><option value="on">On</option><option value="off">Off</option></select> : null}
      <span className="hint">{effectiveMode == null ? "Unknown" : `${settingValue(effectiveMode)} · ${source("reasoning")}`}</span><button type="button" className="quiet-button" disabled={disabled} onClick={() => patch("reasoning", "auto")}>Model default</button>{"reasoning" in value ? <button type="button" className="quiet-button" disabled={disabled} onClick={() => inherit("reasoning")}>Use inherited</button> : null}</div> : null}
    ...
    {!compact ? <div className="setup-form-grid">{([{ key: "temperature", label: "Temperature", min: 0, step: 0.05 }, { key: "max_tokens", label: "Reply limit", min: 1, step: 1 }, { key: "top_p", label: "Top P", min: 0, step: 0.05 }, { key: "min_p", label: "Min P", min: 0, step: 0.01 }] as const).map(item => <label key={item.key}>{item.label}<HoverHelp title={item.label}>{source(item.key)}. Empty uses the inherited setting.</HoverHelp><input type="number" min={item.min} step={item.step} disabled={disabled} value={typeof value[item.key] === "number" ? Number(value[item.key]) : ""} placeholder={current(item.key) == null ? "Default not reported" : String(current(item.key))} onChange={event => event.target.value === "" ? inherit(item.key) : patch(item.key, Number(event.target.value))} /><span className="hint">{current(item.key) == null ? "Default not reported" : `${settingValue(current(item.key))} · ${source(item.key)}`}</span></label>)}</div> : null}
