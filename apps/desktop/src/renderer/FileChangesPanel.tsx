...
export function FileChangesPanel({ runIds, currentRunId, currentRunStatus }: { runIds: string[]; currentRunId?: string | null; currentRunStatus?: string }) {
  ...
  return <section className="file-changes-panel"><div className="file-changes-heading"><h4>Changed project files</h4><button type="button" disabled={busy || loading || !runId} onClick={() => setRevision(value => value + 1)}>Refresh</button></div>{ids.length ? <label>Turn<select aria-label="File changes turn" value={runId} disabled={busy} onChange={event => setRunId(event.target.value)}>{ids.map((id, index) => <option key={id} value={id}>{id === currentRunId ? "Latest turn" : `Earlier turn ${ids.length - index}`}</option>)}</select></label> : null}
    ...
    {selected ? <div className="file-change-detail"><div className="file-changes-heading"><strong>{selected.change.path}</strong><StatusBadge label={selected.change.status} /></div>{selected.change.error ? <Notice tone="error">{selected.change.error}</Notice> : null}{selected.diff != null ? <div className="code-block-wrap"><CopyIconButton text={selected.diff || "No text difference."} label="Copy file difference" /><pre className="file-change-diff" aria-label="File difference">{selected.diff || "No text difference."}</pre></div> : <p className="hint">{selected.diff_unavailable_reason || "A text difference is unavailable."}</p>}<details><summary>Before and after</summary>{(["before", "after"] as const).map(side => <section key={side}><h5>{side === "before" ? "Before" : "After"}</h5>{selected.change[side]?.text != null ? <div className="code-block-wrap"><CopyIconButton text={selected.change[side]?.text ?? ""} label={`Copy ${side === "before" ? "before" : "after"}`} /><pre className="file-change-diff">{selected.change[side]?.text}</pre></div> : <p className="hint">{selected.change[side]?.exists ? "Text preview unavailable." : "File does not exist."}</p>}</section>)}</details>
      <p className="hint">{selected.note}</p>{selected.reversal_available ? confirm ? <div className="notice"><p>Reverse this file change? The current file must still match the recorded result.</p><div className="actions"><button disabled={busy} type="button" onClick={() => void reverse()}>Reverse change</button><button disabled={busy} type="button" onClick={() => setConfirm(false)}>Keep change</button></div></div> : <button disabled={busy} type="button" onClick={() => setConfirm(true)}>Reverse this file change</button> : <p className="hint">{selected.change.reversed_at ? "This change has been reversed." : selected.reversal_unavailable_reason || "This change cannot be reversed."}</p>}
    </div> : null}
  </section>;
}
