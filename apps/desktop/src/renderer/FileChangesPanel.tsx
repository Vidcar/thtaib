...
export function FileChangesPanel({ runIds, currentRunId, currentRunStatus }: { runIds: string[]; currentRunId?: string | null; currentRunStatus?: string }) {
  ...
  return <section className="file-changes-panel"><div className="file-changes-heading"><h4>Changed project files</h4><button type="button" disabled={busy || loading || !runId} onClick={() => setRevision(value => value + 1)}>Refresh</button></div>{ids.length ? <label>Turn<select aria-label="File changes turn" value={runId} disabled={busy} onChange={event => setRunId(event.target.value)}>{ids.map((id, index) => <option key={id} value={id}>{id === currentRunId ? "Latest turn" : `Earlier turn ${ids.length - index}`}</option>)}</select></label> : null}
    ...
    {selected ? <div className="file-change-detail"><div className="file-changes-heading"><strong>{selected.change.path}</strong><StatusBadge label={selected.change.status} /></div>{selected.change.error ? <Notice tone="error">{selected.change.error}</Notice> : null}{selected.diff != null ? <pre className="file-change-diff" aria-label="File difference">{selected.diff || "No text difference."}</pre> : <p className="hint">{selected.diff_unavailable_reason || "A text difference is unavailable."}</p>}<details><summary>Before and after</summary>{(["before", "after"] as const).map(side => <section key={side}><h5>{side === "before" ? "Before" : "After"}</h5>{selected.change[side]?.text != null ? <pre className="file-change-diff">{selected.change[side]?.text}</pre> : <p className="hint">{selected.change[side]?.exists ? "Text preview unavailable." : "File does not exist."}</p>}</section>)}</details>
