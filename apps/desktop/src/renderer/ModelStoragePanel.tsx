...
export function ModelStoragePanel() {
  ...
  return <details className="card"><summary>Model storage</summary>
    <div className="setting-title"><span>Install location</span><Help label="Install location">Changes affect new installations only. Existing models stay at their current locations.</Help></div>
    ...
    <div className="actions"><button type="button" disabled={busy} onClick={() => void action(refresh)}>Refresh sizes</button><button type="button" disabled={busy} onClick={() => setConfirm(true)}>Clean unused downloads</button><Help label="Storage measurements">Disk sizes include separate temporary and installed copies, not RAM or GPU requirements. Cleanup manages this app's unreferenced cache only.</Help></div>
