...
import type { ModelBundle, ResponseRecipe, RunProfile } from "./types";
...
function cardUrl(recipe: ResponseRecipe): string {
  const repo = recipe.source_repo_id.split("/").map(encodeURIComponent).join("/");
  return `https://huggingface.co/${repo}/blob/${encodeURIComponent(recipe.source_revision)}/README.md`;
}
