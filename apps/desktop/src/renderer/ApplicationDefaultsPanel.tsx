...
export function ApplicationDefaultsPanel() {
  ...
  const save = () => { if (pending.current) return; if (visualCompatibilityIssue) { setError(visualCompatibilityIssue); return; } pending.current = true; setBusy(true); setSaved(false); setError(""); void request<SetupConfiguration>("/v1/setup-defaults", { method: "PUT", body: JSON.stringify(value) }).then(next => { setValue(next); setSaved(true); }).catch(failure => setError(errorMessage(failure))).finally(() => { pending.current = false; setBusy(false); }); };
