...
export function ProjectsPanel({ onOpenChat, onAddProject, onProjectChanged, projectRevision = 0, focusProjectId = null, onFocusHandled, contextual = false }: { onOpenChat?: (project: ProjectRecord) => void; onAddProject?: () => void; onProjectChanged?: () => void; projectRevision?: number; focusProjectId?: string | null; onFocusHandled?: () => void; contextual?: boolean }) {
  ...
  const visualCompatibilityIssue = visualSetupCompatibilityIssue(draft.defaults, catalogue);
  ...
  return <section className={`surface workspace-records-surface${contextual ? " project-context" : ""}`}>
    <header className="surface-head"><div className="entity-head"><h2>{contextual ? selected?.name ?? "Project settings" : "Projects"}</h2><HoverHelp title="About projects">A project links an existing folder and its Knowledge. Conversations stay in their original project. Removing the link never deletes source files.</HoverHelp></div>{!contextual ? <button disabled={busy} onClick={() => onAddProject?.()}><Icon name="plus" size={15} /> Add project</button> : null}</header>
    {error ? <Notice tone="error" action={!projects.length ? <button type="button" onClick={() => void action(refresh)}>Retry</button> : undefined}>{error}</Notice> : null}{catalogueError ? <Notice tone="warn">Some knowledge choices could not load: {catalogueError}</Notice> : null}{message ? <p role="status" className="hint">{message}</p> : null}
    <div className="workspace-records-layout"><aside className="workspace-record-list" aria-label="Your projects">{loading ? <p className="hint">Loading projects…</p> : !projects.length ? <EmptyState title="No projects yet">Add a folder to keep its chats and Knowledge together.</EmptyState> : <ul className="nav-list">{projects.map(project => <li key={project.id}><button type="button" disabled={busy} className={project.id === selectedId ? "nav-item active" : "nav-item"} onClick={() => setSelectedId(project.id)}><span className="nav-item-title">{project.name}</span><span className="nav-item-meta">{project.missing ? "Folder unavailable" : project.path}{drafts[project.id] ? " · Unsaved changes" : ""}</span></button></li>)}</ul>}</aside>
    <div className="workspace-record-detail">{selected ? <>
      <section className="card"><div className="section-heading"><h3>{selected.name}</h3>{onOpenChat ? <button disabled={busy || selected.missing} onClick={() => onOpenChat(selected)}><Icon name="chat" size={15} /> New chat in project</button> : null}</div><p className="workspace-path">{selected.path}</p>{selected.missing ? <Notice tone="warn">This folder is unavailable. Reconnect its drive or restore the folder before using file tools.</Notice> : null}</section>
      <details className="card" open={contextual || undefined}><summary>Project knowledge <span className="hint">{Object.values(draft.defaults).some(value => value != null) ? "Custom choices" : "No saved knowledge"}</span></summary><form className="workspace-editor" onSubmit={event => { event.preventDefault(); if (visualCompatibilityIssue) { setError(visualCompatibilityIssue); return; } if (draft.name.trim()) void action(async () => { await workspaceApi.updateProject(selected.id, { name: draft.name.trim(), defaults: draft.defaults }); await refresh(selected.id); onProjectChanged?.(); setDrafts(current => { const next = { ...current }; delete next[selected.id]; return next; }); setMessage("Project knowledge saved for future work."); }); }}><label>Name<input required maxLength={200} value={draft.name} disabled={busy} onChange={event => updateDraft({ name: event.target.value })} /></label><SetupConfigurationEditor value={draft.defaults} onChange={defaults => updateDraft({ defaults })} disabled={busy} catalogue={catalogue} scope="project" projectId={selected.id} /><div className="actions"><button className="primary-button" disabled={busy || !draft.name.trim() || Boolean(visualCompatibilityIssue)}>Save project</button>{drafts[selected.id] ? <button type="button" disabled={busy} onClick={() => setDrafts(current => { const next = { ...current }; delete next[selected.id]; return next; })}>Discard edits</button> : null}</div></form></details>
      <section className="card project-files"><div className="section-heading"><h3>Files</h3>{folder ? <button type="button" disabled={filesLoading} onClick={() => setFolder(folder.replaceAll("\\", "/").split("/").slice(0, -1).join("/"))}>Up one folder</button> : null}</div>{folder ? <p className="hint workspace-path">{folder}</p> : null}{filesLoading ? <p className="hint" role="status">Reading folder…</p> : fileError ? <Notice tone="error" action={<button type="button" onClick={() => setFolder("")}>Return to project folder</button>}>{fileError}</Notice> : files ? <ul className="plain-list">{files.entries.map(file => <li key={file.path}>{file.kind === "directory" ? <button type="button" onClick={() => setFolder(file.path)}><Icon name="folder" size={14} /><span>{file.name}</span></button> : <span><Icon name="files" size={14} /><span>{file.name}</span></span>}<small className="hint">{file.kind === "directory" ? "Folder" : formatBytes(file.size_bytes)}</small></li>)}{!files.entries.length ? <li className="hint">This folder is empty.</li> : null}</ul> : null}</section>
      <LifecycleAction key={selected.id} path={`/v1/projects/${selected.id}`} name={selected.name} label="Remove project link" confirmLabel="Remove link" disabled={busy} onBusyChange={setBusy} onComplete={async () => { await refresh(); onProjectChanged?.(); setMessage("Project link removed. Source files are unchanged."); }} />
    </> : loading ? <p className="hint" role="status">Loading project…</p> : !error ? <EmptyState title="No project selected"><button type="button" onClick={onAddProject}><Icon name="plus" size={14} />Add project</button></EmptyState> : null}</div></div>
  </section>;
}
