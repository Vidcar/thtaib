...
export function ProjectsPanel({ onOpenChat }: { onOpenChat?: (project: ProjectRecord) => void }) {
  ...
  useEffect(() => { let cancelled = false; void workspaceApi.projects().then(next => { if (!cancelled) { setProjects(next); setSelectedId(next[0]?.id ?? ""); setCreating(!next.length); } }).catch(failure => { if (!cancelled) setError(errorMessage(failure)); }).finally(() => { if (!cancelled) setLoading(false); }); return () => { cancelled = true; }; }, []);
  ...
  async function browse() { try { const selectedPath = await window.workbench?.selectPath?.("folder"); if (selectedPath) setPath(selectedPath); } catch (failure) { setError(errorMessage(failure)); } }
  return <section className="surface workspace-records-surface">
    <header className="surface-head"><div className="entity-head"><h2>Projects</h2><HoverHelp title="About projects">A project links an existing folder with its own defaults and Knowledge. Conversations stay in their original project. Removing the link never deletes source files.</HoverHelp></div><button disabled={busy} onClick={() => setCreating(true)}><Icon name="plus" size={15} /> Add project</button></header>
    {error ? <Notice tone="error">{error}{!projects.length && !creating ? <button onClick={() => void action(refresh)}>Retry</button> : null}</Notice> : null}{catalogueError ? <Notice tone="warn">Some defaults could not load: {catalogueError}</Notice> : null}{message ? <p role="status" className="hint">{message}</p> : null}
    <div className="workspace-records-layout"><aside className="workspace-record-list" aria-label="Your projects">{loading ? <p className="hint">Loading projects…</p> : !projects.length ? <p className="hint">Add a folder to keep its chats, defaults and Knowledge together.</p> : <ul className="nav-list">{projects.map(project => <li key={project.id}><button type="button" disabled={busy} className={!creating && project.id === selectedId ? "nav-item active" : "nav-item"} onClick={() => { setSelectedId(project.id); setCreating(false); }}><span className="nav-item-title">{project.name}</span><span className="nav-item-meta">{project.missing ? "Folder unavailable" : project.path}{drafts[project.id] ? " · Unsaved changes" : ""}</span></button></li>)}</ul>}</aside>
    <div className="workspace-record-detail">{creating ? <form className="card workspace-editor" onSubmit={event => { event.preventDefault(); if (path.trim()) void action(async () => { const next = await workspaceApi.createProject(path.trim(), name.trim() || undefined); await refresh(next.id); setCreating(false); setPath(""); setName(""); setMessage("Project added. Its source files stay in the selected folder."); }); }}><h3>Add a project folder</h3><label>Folder<input value={path} required disabled={busy} onChange={event => setPath(event.target.value)} placeholder="Choose an existing project folder" /></label><button type="button" disabled={busy || !window.workbench?.selectPath} onClick={() => void browse()}><Icon name="folder" size={15} /> Choose folder</button><label>Name (optional)<input maxLength={200} value={name} disabled={busy} onChange={event => setName(event.target.value)} placeholder="Uses the folder name" /></label><div className="actions"><button className="primary-button" disabled={busy || !path.trim()}>Add project</button>{projects.length ? <button type="button" disabled={busy} onClick={() => setCreating(false)}>Cancel</button> : null}</div></form> : selected ? <>
