...
function usableRecipe(value: unknown): ResponseRecipe | null {
  ...
  if (recipe.reasoning !== "on" && recipe.reasoning !== "off") return null;
...
function recipeSummary(recipe: ResponseRecipe): string {
  ...
  return [`Thinking ${recipe.reasoning === "off" ? "off" : "on"}`, ...values].join(" · ");
}
