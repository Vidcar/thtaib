...
export function DeploymentsPanel({
  ...
  return <section className="model-configuration">
    ...
    {selected ? <section className="card model-setup"><div className="section-heading"><h3>Configuration</h3><span className="model-edit-state" data-dirty={dirty.current}>{dirty.current ? "Edits to save" : "Saved configuration"}</span></div><form ref={formRef} className="model-settings" onSubmit={event => {
      ...
      {creatingVariant ? <div className="actions"><label>Variant name<input value={variantName} onChange={event => setVariantName(event.target.value)} /></label><button type="button" disabled={Boolean(busy) || !variantName.trim()} onClick={() => void action("variant", () => saveConfiguration(true))}>Save variant</button><button type="button" onClick={() => setCreatingVariant(false)}>Cancel</button></div> : null}
