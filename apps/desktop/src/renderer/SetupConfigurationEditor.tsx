...
import { ApprovalModeControl, approvalModeOf } from "./ApprovalModeControl";
...
export function SetupConfigurationEditor({ value, onChange, catalogue, disabled = false, requirements = false }: { value: SetupConfiguration; onChange: (value: SetupConfiguration) => void; catalogue: SetupCatalogue; disabled?: boolean; requirements?: boolean }) {
  ...
  return <div className="setup-configuration-editor">
    ...
    <details className="setup-options"><summary>Tools and connections</summary><SelectionList title="Tools" values={value.presented_tools} options={catalogue.tools} disabled={disabled} onChange={presented_tools => patch({ presented_tools })} /><ApprovalModeControl value={approvalModeOf(value.approval_mode)} disabled={disabled} onChange={approval_mode => patch({ approval_mode })} /><SelectionList title="Connections" values={value.connection_ids} options={catalogue.connections} disabled={disabled} onChange={connection_ids => patch({ connection_ids })} /></details>
