...
export function SetupConfigurationEditor({ value, onChange, catalogue, disabled = false, requirements = false, scope = "agent", projectId = null }: { value: SetupConfiguration; onChange: (value: SetupConfiguration) => void; catalogue: SetupCatalogue; disabled?: boolean; requirements?: boolean; scope?: "application" | "project" | "agent"; projectId?: string | null }) {
  ...
  return <div className="setup-configuration-editor">
    ...
    <label><span className="setting-title">Access<HoverHelp title="Access">Ask pauses before edits, shell commands and external effects. Full access permits enabled tools. Saved permissions are named exceptions. Host shell runs with your Windows user’s authority.</HoverHelp></span><select value={value.approval_mode ?? ""} disabled={disabled} onChange={event => patch({ approval_mode: (event.target.value || null) as SetupConfiguration["approval_mode"] })}><option value="">{inheritedLabel(facts.approval_mode)}</option><option value="ask">Ask for approval</option><option value="full_access">Full access</option></select></label></div>
    <label><span className="setting-title">Instructions<HoverHelp title="Instructions">Adds guidance at this level. Instructions never grant additional permissions.</HoverHelp></span><textarea value={value.instructions ?? ""} disabled={disabled} onChange={event => patch({ instructions: event.target.value || null })} placeholder={scope === "agent" ? "How should this agent work?" : scope === "project" ? "Guidance for conversations in this project" : "Guidance for new conversations"} rows={4} /></label>
