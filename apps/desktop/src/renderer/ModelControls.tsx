...
export function Choice({ id, label, help, flag, value, options, onChange, custom = true, min = 0, max, step = 1, disabled = false, meta, segmented = false }: {
  ...
  custom?: boolean; min?: number; max?: number; step?: number; disabled?: boolean; meta?: string; segmented?: boolean;
}) {
  ...
  if (binary) return <div className="model-field"><CompactSwitch label={label} description={help} meta={meta} checked={value === "on"} disabled={disabled} onChange={checked => onChange(checked ? "on" : "off")} /></div>;
  if (segmented) return <div className="model-field"><SegmentedChoice label={label} value={value} options={options} onChange={onChange} disabled={disabled} description={help} meta={meta} /></div>;
  ...
  if (custom && numeric.length >= 3 && value !== "custom" && value !== "" && Number.isFinite(Number(value))) {
    ...
    return <div className="model-field"><CompactSlider label={label} description={help} value={Number(value)} values={numeric} disabled={disabled} formatValue={number => options.find(option => option.value === String(number))?.label ?? (label.toLowerCase().includes("context") ? `${tokenLabel(number)} tokens` : number.toLocaleString())} onChange={number => onChange(String(number))} /><div className="actions"><input aria-label={`Exact ${label.toLowerCase()}`} type="number" min={min} max={max} step={step} disabled={disabled} value={value} onChange={event => onChange(event.target.value || "custom")} />{automatic.map(option => <button key={option.value} type="button" className="quiet-button" disabled={disabled} onClick={() => onChange(option.value)}>{option.label}</button>)}</div></div>;
  }
  const isCustom = value === "custom" || !options.some(option => option.value === value);
  return <div className="model-field"><div className="setting-title"><label htmlFor={id}>{label}</label><Help label={label} flag={flag}>{help}</Help>{meta ? <small className="control-provenance">{meta}</small> : null}</div>
