...
export type VariantPresentation = { variant: SchemaHubVariant; quant: string; bits: number | null; family: string };
...
export function presentVariant(variant: SchemaHubVariant): VariantPresentation {
  ...
  return { variant, quant, bits, family: bits == null ? "Unknown" : `${bits}-bit` };
}
